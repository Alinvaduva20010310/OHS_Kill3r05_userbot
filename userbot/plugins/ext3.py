...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "Andrex"
...
async def _(event):
    ...
    if not event.reply_to_msg_id:
       await event.edit(f"`{DEFAULTUSER}:`**⚠️ Rispondi a foto/sticker**")
       return
    ...
    if reply_message.sender.bot:
       await event.edit(f"`{DEFAULTUSER}:`**⚠️ Rispondi ad un'utente e non al bot.**")
       return
    ...
    async with bot.conversation(chat) as conv:
          try:     
              ...
              await event.reply("```❗Attenzione, sblocca @hazmat_suit_bot ```")
              return
          if response.text.startswith("Forward"):
             await event.edit(f"`{DEFAULTUSER}:`**⚠️ Errore di privacy**")
          else:
